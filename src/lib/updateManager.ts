...
export class UpdateManager {
  ...
  async checkForUpdates(): Promise<UpdateInfo[]> {
    ...
    for (const workflowName of manifest.components.workflows) {
      const updateInfo = await this.checkComponentUpdate(
        "workflow",
        workflowName
      );
      if (updateInfo) {
  ...
  async updateComponent(
    ...
    logger.success(
      `Updated ${type} '${name}' to version ${updateInfo.latestVersion}`
    );
  }
  ...
  async updateAll(force = false): Promise<void> {
    ...
    for (const update of updates) {
      try {
        ...
        logger.error(
          `Failed to update ${update.type} '${update.component}': ${error}`
        );
      }
    }
  }
   ...
   */
  private async checkComponentUpdate(
    type: "mode" | "workflow",
    name: string
    ...
    if (
      currentVersion &&
      currentVersion.version === templateVersion &&
      !hasLocalChanges
    ) {
      return null;
    }
   ...
   */
  private async getComponentVersion(
    type: "mode" | "workflow",
    name: string
   ...
   */
  private async extractVersionFromTemplate(
    _type: "mode" | "workflow",
    _name: string
  ): Promise<string> {
